...
export default class InventoryPage extends BasePage {
    ...
    private readonly Elements = {
        ...
        cartLink: this.page.locator("a[data-test='shopping-cart-link']"),
        ...
        priceLabel: this.page.locator(".inventory_item_price"),
    }
    ...
    async isLoaded() {
        ...
        await expect(this.Elements.inventoryItem.first()).toBeVisible({ timeout: 5000 });
    ...
    async getAllProductNames(): Promise<string[]> {
        ...
        const names = this.Elements.productNames;
    ...
    async sortByVisibleText(option: string) {
        await this.Elements.sortSelect.selectOption({ label: option });
        console.log(`Sorted by: ${option}`);
    }

    async getAllPricesInOrder(): Promise<number[]> {
        const prices: number[] = [];
        const priceElements = this.Elements.priceLabel;
